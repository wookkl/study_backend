...
- 가급적 음수 증가값은 피한다.
- 한 슬라이스 안에서 시작, 끝, 증가값을 함께 사용하지 않는다. 세 파라미터를 사용해야 하는 경우 두 번 대입을 사용하거나 itertoolsdml `islice`를 사용한다.
...
## **missing**을 사용해 키에 따라 다른 디폴트 값을 생성하는 방법을 알아두라
...
**missing** 특별 메서드를 구현하면 키가 없는 경우를 처리하는 로직을 커스텀할 수 있다.
